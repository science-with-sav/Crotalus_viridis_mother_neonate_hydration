...
SLR_dat <- full_dat %>% dplyr::filter(Preg_Stage == "DAB")
...
```{r SMI equation}
r <- sqrt(0.4008) # Pearson's correlection coefficient (sqrt of R-squared)
b_OLS <- 2.339 # regression slope
b_SMA <- b_OLS/r
b_SMA
```
...
full_dat2 <- full_dat %>%
  # compute SMI
  mutate(SMI = Mass_g * ((81.31667/SVL_cm) ^ 3.694591))
