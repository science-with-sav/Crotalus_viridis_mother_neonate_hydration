...
SLR_dat <- full_dat %>% dplyr::filter(Preg_Stage == "1WAC")
...
```{r SMI equation}
r <- sqrt(0.4594) # Pearson's correlection coefficient (sqrt of R-squared)
b_OLS <- 1.721 # regression slope
b_SMA <- b_OLS/r
b_SMA
```
...
full_dat2 <- full_dat %>%
  # compute SMI
  mutate(SMI = Mass_g * ((78.685/SVL_cm) ^ 2.539133))
