...
SLR_dat <- full_dat %>% dplyr::filter(Preg_Stage == "1WAB")
...
```{r SMI equation}
r <- sqrt(0.7209) # Pearson's correlection coefficient (sqrt of R-squared)
b_OLS <- 1.6175 # regression slope
b_SMA <- b_OLS/r
b_SMA
```
...
full_dat2 <- full_dat %>%
  # compute SMI
  mutate(SMI = Mass_g * ((77.375/SVL_cm) ^ 1.905052))
